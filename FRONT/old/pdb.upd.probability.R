...
for(i in 1:length(file$alpha)){
  back_score[i] <- scoring_up_wochi(evaluate = file[i,1:7], bd = bayesian_data)
}
...
for(i in 1:length(back_score)){
  normal_data[i] <- (back_score[i] - min_max_scoring_update[[1]]) / (min_max_scoring_update[[2]] - min_max_scoring_update[[1]])
}
