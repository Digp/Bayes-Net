...
#vpath <- "/Users/eric1999/Desktop/Bayessian_RNA_val/FRONT/1a9n.pdb"
#rpath <- "/Users/eric1999/Desktop/Bayessian_RNA_val/FRONT/Data/"
#dpath <- "/Users/eric1999/Desktop/Bayessian_RNA_val/BACK/0.get/Data/"
#spath <- "/Users/eric1999/Desktop/Bayessian_RNA_val/FRONT/New_PDBs/"
opt$pdb <- "/Users/eric1999/Desktop/Bayessian_RNA_val/Mixed_Model/BACK/Outliers_PDB/3zjv.pdb"
opt$pdb <- NULL
####Read the file
if(!is.null(opt$pdb)){
...
for(i in 1:length(file$alpha)){
  back_score[i] <- scoring_wochi(evaluate = file[i,1:7], bd = bayesian_data)
}
...
for(i in 1:length(back_score)){
  normal_data[i] <- (back_score[i] - min_max_scoring[[1]]) / (min_max_scoring[[2]] - min_max_scoring[[1]])
}

normal_data <- round(normal_data, 3)


## Save new pdb file
pdb_new <- replace_pdb(normal_data, input, "b")

write.pdb(pdb = pdb_new, file= paste(spath, sep=""), segid = pdb_new$atom$entid)
