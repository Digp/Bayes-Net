...
scoring_complete <- xlapply(1:length(files_pdb), mc.cores=opt$cores, function(m) {
  ####Read the file
  print(files_pdb[m])
  
  #file <- measureNuc_light(read.pdb(paste(opt$pdb, files_pdb[m], sep = "")))[, c("alpha", "beta", "gamma", "delta", "epsilon","zeta","chi")]
  ...
  for(i in 1:length(file$alpha)){
    bayes_back[i] <- round(scoring_bayes(ntinfo_disc = file_disc[i,], bd = bayesian_data,ntinfo = file[i,])$total, 3)
  }
