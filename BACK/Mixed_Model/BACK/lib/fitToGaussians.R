...
#    require(mclust)
    ...
    seq <- seq(0, 360, by=0.001)
    max_dens <- max(unlist(lapply(seq, FUN=function(x)
                                sum(dnorm(x,mean,sd)*prop))))
    
    #data <- data.frame(prop,mean,sd)
    
    #max_dens <- function(data = NULL){
    #    data_store <- vector("list", gaussians)
    #    for(i in 1:gaussians){
    #        data_store[[i]] <- 0
    #        for(m in 1:length(seq)){
    #            calc <- dnorm(seq[m], data$mean[i], data$sd[i])*data$prop[i]
    #            if(calc > data_store[[i]]){
    #                data_store[[i]] <- calc
    #            }
    #        }
    #    }
