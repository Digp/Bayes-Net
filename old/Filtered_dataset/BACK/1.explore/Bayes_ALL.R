...
for(i in 1:length(ntinfo_m$alpha)){
  data_score_upd[i] <- minipack::scoring_up_wochi(evaluate = ntinfo_m[i,], bd = bayesian_data)
}
...
Data_analysis <- cbind(PDBID_id_dssr, data_all, data_normalized, data_score_upd, data_normal_upd, bulk_cybernating$Scoring_cybernating,
...
names(Data_analysis) <- namy


#Save the data
save(Data_analysis, file = paste(vpath, "Scoring_Analysis.RData"))


###############MAKE THE PLOTTING

mpath <- "/Users/eric1999/Desktop/Bayessian_RNA_val/Filtered_dataset/BACK/1.explore/Plots/ALL_versus_delta/"
lpath <- "/Users/eric1999/Desktop/Bayessian_RNA_val/Filtered_dataset/BACK/1.explore/Plots/ALL_versus_delta/"
