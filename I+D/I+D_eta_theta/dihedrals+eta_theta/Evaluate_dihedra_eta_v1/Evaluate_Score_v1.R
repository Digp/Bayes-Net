...
source(paste(impath, "Scoring_dihedral_all.R", sep=""))
source(paste(impath, "probability_all.R", sep=""))

load(paste(impath_2, "ntinfo_dihedrals_eta.RData", sep=""))
load(paste(impath, "Bayesian_data_4_degree_v1.RData", sep=""))
...
for(i in 1:length(ntinfo_m$eta)){
  data_all[i] <- scoring_all(evaluate = ntinfo_l[i,], bd = bayesian_data)
}
